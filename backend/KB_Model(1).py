...
class EnhancedStalenessDetector:
    ...
    def predict_single(self, ci_data):
        ...
        for scenario_id, scenario_pattern in self.scenario_patterns.items():
            ...
            if match_score > 0.4:  # More selective scenario matching
                confidence = min(0.92, scenario_pattern['avg_confidence'] * match_score * 0.92)  # More conservative confidence
        ...
        # Determine staleness - More precise decision making with final sanity check
        is_stale = (max_confidence > 0.78 and 
                   self._validate_precise_staleness_conditions(features) and
                   self._has_sufficient_evidence(features) and
                   self._final_staleness_sanity_check(features))
    ...
    def _extract_temporal_features(self, ci_data):
        ...
        # Temporal categories - Slightly more strict
        features['no_activity_3_months'] = features['days_since_last_activity'] > 100  # Increased from 90 to 100 days
        features['no_activity_6_months'] = features['days_since_last_activity'] > 190  # Increased from 180 to 190 days
        features['no_activity_1_year'] = features['days_since_last_activity'] > 375   # Increased from 365 to 375 days
        features['no_owner_activity_30_days'] = features['days_since_owner_activity'] > 40  # Increased from 30 to 40 days
    ...
    def _extract_behavioral_features(self, ci_data):
        ...
        # Behavioral patterns - More precise thresholds
        features['multiple_users_active'] = len(other_users) > 1  # Keep original
        features['owner_minimal_activity'] = features['owner_activity_ratio'] < 0.20 and features['owner_activity_count'] <= 2  # More precise: 0.25→0.20, count ≤2
        features['predominantly_system_activities'] = features['system_activity_ratio'] > 0.70  # Increased from 0.65 to 0.70
        features['unclear_ownership'] = features['multiple_users_active'] and features['owner_activity_ratio'] < 0.35  # Reduced from 0.4 to 0.35
    ...
    def _validate_precise_staleness_conditions(self, features):
        """
        Precise validation with stronger evidence requirements to reduce false positives
        """
        # Critical indicators - at least ONE must be present
        critical_indicators = 0
        
        # Critical 1: Account definitely problematic
        if (features.get('account_terminated', False) or 
            features.get('account_not_found', False)):
            critical_indicators += 1
        
        # Critical 2: Clear vendor/generic account issues
        if (features.get('generic_account_indicator', False) or
            features.get('vendor_account_indicator', False)):
            critical_indicators += 1
            
        # Critical 3: Owner completely absent AND significant time passed
        if (features.get('owner_has_no_activity', False) and 
            features.get('days_since_owner_activity', 0) > 150):  # 5 months
            critical_indicators += 1
        
        # Critical 4: Severe ownership mismatch
        if (features.get('owner_activity_ratio', 1.0) < 0.15 and 
            features.get('top_other_user_ratio', 0) > 0.45):
            critical_indicators += 1
        
        # Supporting evidence - need at least 2 of these
        supporting_evidence = 0
        
        # Support 1: Multiple users clearly active
        if (features.get('multiple_users_active', False) and 
            features.get('other_users_count', 0) >= 2):
            supporting_evidence += 1
        
        # Support 2: No recent owner activity at all
        if features.get('no_recent_owner_activity', False):
            supporting_evidence += 1
        
        # Support 3: Long period without any significant activity
        if features.get('days_since_last_activity', 0) > 200:  # 6.5 months
            supporting_evidence += 1
            
        # Support 4: Non-owner making ownership changes
        if features.get('non_owner_ownership_changes', 0) > 0:
            supporting_evidence += 1
        
        # Support 5: Owner minimal activity pattern established
        if features.get('owner_minimal_activity', False):
            supporting_evidence += 1
        
        # Require at least 1 critical indicator AND at least 2 supporting pieces of evidence
        return critical_indicators >= 1 and supporting_evidence >= 2
    
    def _has_sufficient_evidence(self, features):
        """
        Ensure we have enough activity data to make a reliable decision
        """
        # Need reasonable amount of audit data
        min_activities = features.get('total_activity_count', 0) >= 4
        
        # Either significant time has passed OR we have clear account issues
        sufficient_time = features.get('days_since_last_activity', 0) > 60
        account_issues = (features.get('account_terminated', False) or 
                         features.get('account_not_found', False) or
                         features.get('generic_account_indicator', False) or
                         features.get('vendor_account_indicator', False))
        
        return min_activities and (sufficient_time or account_issues)
    
    def _final_staleness_sanity_check(self, features):
        """
        Final sanity check to prevent false positives in edge cases
        """
        # Red flag: Owner still has recent significant activity
        if (features.get('recent_owner_activities', 0) > 2 and 
            features.get('owner_activity_ratio', 0) > 0.3):
            return False
        
        # Red flag: Very low total activity (might be normal for this CI)
        if features.get('total_activity_count', 0) < 5:
            # Only allow if we have strong account issues
            strong_account_issues = (features.get('account_terminated', False) or 
                                   features.get('account_not_found', False))
            if not strong_account_issues:
                return False
        
        # Red flag: Owner activity ratio is reasonable and they're still active
        if (features.get('owner_activity_ratio', 0) > 0.4 and 
            features.get('days_since_owner_activity', 999) < 90):
            return False
            
        # Red flag: No clear alternative user taking over
        if (features.get('top_other_user_ratio', 0) < 0.25 and 
            not features.get('account_terminated', False) and
            not features.get('account_not_found', False)):
            return False
