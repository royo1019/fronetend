...
class ScenarioKnowledgeBase:
    ...
    def _calculate_match_score(self, features: Dict, audit_patterns: List[Dict], scenario: Dict) -> float:
        ...
        # Check pattern matches with more balanced scoring
        ...
        # Temporal pattern matching - more conservative
        if pattern.get('extended_absence') and features.get('days_since_owner_activity', 0) > 365:  # 1 year instead of 180 days
            score += 0.15
...
class EnhancedStalenessDetector:
    ...
    def predict_single(self, ci_data):
        ...
        for scenario_id, scenario_pattern in self.scenario_patterns.items():
            ...
            if match_score > 0.3:  # Lowered threshold for scenario matching
                # More conservative confidence calculation
                confidence = min(0.85, scenario_pattern['avg_confidence'] * match_score * 0.85)
        ...
        # Determine staleness - FIXED: More balanced decision making
        is_stale = self._determine_staleness_balanced(features, max_confidence)
        ...
        if kb_scenario and kb_confidence > 0.1:
            ...
            explanation = "No clear staleness pattern identified"
        
        # Recommend new owners
        new_owner_recommendations = self._recommend_new_owners(ci_data)
        
        # Ensure all return values are properly formatted and not None
        kb_scenario_name = None
        if kb_scenario and kb_scenario in self.kb.scenarios:
            kb_scenario_name = self.kb.scenarios[kb_scenario]['name']
        ...
        return {
            ...
            'kb_scenario_name': kb_scenario_name,
            'kb_confidence': kb_confidence if kb_confidence > 0 else 0.0,
            'explanation': explanation if explanation else "No explanation available",
            'new_owner_recommendations': new_owner_recommendations if new_owner_recommendations else [],
            'features': features
        }
    
    def _determine_staleness_balanced(self, features, scenario_confidence):
        """
        FIXED: Balanced staleness determination that considers both positive and negative indicators
        """
        # Initialize staleness score
        staleness_score = 0.0
        
        # HIGH PRIORITY POSITIVE INDICATORS (Strong evidence of staleness)
        # Account issues (highest weight)
        if features.get('account_terminated', False):
            staleness_score += 0.35
        elif features.get('account_not_found', False):
            staleness_score += 0.30
        
        # Generic/vendor accounts
        if features.get('generic_account_indicator', False):
            staleness_score += 0.25
        elif features.get('vendor_account_indicator', False):
            staleness_score += 0.20
        
        # Complete owner absence
        if features.get('owner_has_no_activity', False):
            days_no_activity = features.get('days_since_owner_activity', 0)
            if days_no_activity > 365:  # 1 year
                staleness_score += 0.30
            elif days_no_activity > 180:  # 6 months
                staleness_score += 0.20
            elif days_no_activity > 90:   # 3 months
                staleness_score += 0.10
        
        # MEDIUM PRIORITY INDICATORS
        # Ownership mismatch patterns
        owner_ratio = features.get('owner_activity_ratio', 1.0)
        other_user_ratio = features.get('top_other_user_ratio', 0)
        
        if owner_ratio < 0.1 and other_user_ratio > 0.5:  # Clear ownership transfer
            staleness_score += 0.20
        elif owner_ratio < 0.2 and other_user_ratio > 0.4:  # Likely ownership transfer
            staleness_score += 0.15
        elif owner_ratio < 0.3 and other_user_ratio > 0.3:  # Possible ownership transfer
            staleness_score += 0.10
        
        # Multiple active users (organizational change indicator)
        if features.get('multiple_users_active', False) and features.get('other_users_count', 0) > 2:
            staleness_score += 0.10
        
        # Non-owner making ownership changes
        if features.get('non_owner_ownership_changes', 0) > 0:
            staleness_score += 0.15
        
        # LOW PRIORITY INDICATORS
        # Long periods without activity
        days_since_activity = features.get('days_since_last_activity', 0)
        if days_since_activity > 365:
            staleness_score += 0.10
        elif days_since_activity > 180:
            staleness_score += 0.05
        
        # NEGATIVE INDICATORS (Evidence against staleness)
        negative_score = 0.0
        
        # Owner still reasonably active
        if owner_ratio > 0.5:
            negative_score += 0.25
        elif owner_ratio > 0.3:
            negative_score += 0.15
        
        # Recent owner activity
        recent_owner_activities = features.get('recent_owner_activities', 0)
        if recent_owner_activities > 5:
            negative_score += 0.20
        elif recent_owner_activities > 2:
            negative_score += 0.10
        
        # Owner account active and found
        if features.get('account_active', False) and features.get('owner_in_user_directory', False):
            negative_score += 0.10
        
        # Low total activity (might be normal for this CI)
        total_activities = features.get('total_activity_count', 0)
        if total_activities < 5:
            negative_score += 0.15
        
        # Predominantly system activities (automated, not necessarily stale)
        if features.get('system_activity_ratio', 0) > 0.8:
            negative_score += 0.10
        
        # Calculate final score
        final_score = max(0, staleness_score - negative_score)
        
        # Add scenario confidence if available
        if scenario_confidence > 0:
            final_score = min(1.0, final_score + (scenario_confidence * 0.2))
        
        # DECISION THRESHOLDS - More conservative
        is_stale = final_score > 0.6  # Increased from previous lower threshold
        
        # Additional validation - require minimum evidence
        if is_stale:
            # Must have at least one strong indicator OR combination of medium indicators
            strong_indicators = (
                features.get('account_terminated', False) or
                features.get('account_not_found', False) or
                features.get('generic_account_indicator', False) or
                features.get('vendor_account_indicator', False) or
                (features.get('owner_has_no_activity', False) and features.get('days_since_owner_activity', 0) > 180)
            )
            
            medium_indicators = (
                (owner_ratio < 0.2 and other_user_ratio > 0.3) or
                features.get('non_owner_ownership_changes', 0) > 0 or
                (features.get('multiple_users_active', False) and features.get('other_users_count', 0) > 1)
            )
            
            # Require at least one strong indicator OR at least two medium indicators
            if not (strong_indicators or medium_indicators):
                is_stale = False
    ...
    def _extract_temporal_features(self, ci_data):
        ...
        # Temporal categories - More balanced thresholds
    ...
    def _extract_behavioral_features(self, ci_data):
        ...
        # Behavioral patterns - More balanced thresholds
        features['multiple_users_active'] = len(other_users) > 1
        features['owner_minimal_activity'] = features['owner_activity_ratio'] < 0.25 and features['owner_activity_count'] <= 3
        features['predominantly_system_activities'] = features['system_activity_ratio'] > 0.75
        features['unclear_ownership'] = features['multiple_users_active'] and features['owner_activity_ratio'] < 0.4
...
# Keep all the remaining functions as they were...
...
def main():
    ...
    for ci in stale_ci_report:
        scenario = ci.get('scenario', 'Unknown')
        # Handle None values
        if scenario is None or scenario == 'None':
            scenario = 'Unknown'
...
def visualize_scenario_distribution(stale_ci_report):
    ...
    # Count scenarios - handle None values
    scenario_counts = {}
    for ci in stale_ci_report:
        scenario = ci.get('scenario', 'Unknown')
        # Handle None values
    ...
    sorted_data = sorted(scenario_counts.items(), key=lambda x: x[1], reverse=True)[:15]
    
    if not sorted_data:
        print("No scenario data to display")
        return
        
    scenarios, counts = zip(*sorted_data)
    
    # Truncate long scenario names for better display
    display_scenarios = [s[:50] + '...' if len(s) > 50 else s for s in scenarios]
    
    plt.barh(display_scenarios, counts, color='steelblue')
    ...
    other_count = sum(count for _, count in sorted_data[10:]) if len(sorted_data) > 10 else 0
    
    labels = [s[0][:30] + '...' if len(s[0]) > 30 else s[0] for s in top_scenarios]
    sizes = [s[1] for s in top_scenarios]
    ...
    print(f"Total scenarios identified: {len(scenario_counts)}")
    print(f"Top 10 scenarios:")
    for i, (scenario, count) in enumerate(sorted_data[:10], 1):
        percentage = count / sum(scenario_counts.values()) * 100
        print(f"  {i:2d}. {scenario}: {count} CIs ({percentage:.1f}%)")


# Function to test model balance and provide debugging
def test_model_balance(detector, test_data, sample_size=50):
    """Test the model on a sample to check for balance"""
    print(f"\n🧪 Testing Model Balance on {sample_size} samples...")
    
    sample_data = test_data[:sample_size] if len(test_data) >= sample_size else test_data
    ...
    for item in sample_data:
        ...
        if result['is_stale']:
            ...
            predictions['non_stale'] += 1
        
        predictions['details'].append({
            'ci_id': item['ci_id'],
            'true_label': item['true_label'],
            'predicted_stale': result['is_stale'],
            'confidence': result['confidence'],
            'explanation': result['explanation'][:100] + '...' if len(result['explanation']) > 100 else result['explanation']
        })
    
    print(f"📊 Prediction Results on {len(sample_data)} samples:")
    print(f"  Predicted Stale: {predictions['stale']} ({predictions['stale']/len(sample_data)*100:.1f}%)")
    print(f"  Predicted Non-Stale: {predictions['non_stale']} ({predictions['non_stale']/len(sample_data)*100:.1f}%)")
    
    # Show some examples of both predictions
    print(f"\n🔍 Sample Predictions:")
    stale_examples = [d for d in predictions['details'] if d['predicted_stale']][:3]
    non_stale_examples = [d for d in predictions['details'] if not d['predicted_stale']][:3]
    
    print(f"\nPredicted as STALE:")
    for example in stale_examples:
        print(f"  CI {example['ci_id']}: Confidence {example['confidence']:.3f}")
        print(f"    True Label: {example['true_label']}")
        print(f"    Explanation: {example['explanation']}")
    
    print(f"\nPredicted as NON-STALE:")
    for example in non_stale_examples:
        print(f"  CI {example['ci_id']}: Confidence {example['confidence']:.3f}")
        print(f"    True Label: {example['true_label']}")
        print(f"    Explanation: {example['explanation']}")
...
if __name__ == "__main__":
    ...
    test_predictions = test_model_balance(detector, test_data, sample_size=100)
